...
const AllCourses: FC<Props> = ({ isTeam }) => {
  ...
  // Ensure data and data.users are both defined
  if (data && data.users) {
    const userData = isTeam
      ? data.users.filter((item: any) => item.role === "admin")
      : data.users;
  
    // Use map to transform the data directly into rows
    ...
    rows.push(...rowData); // Spread rowData into rows array
  }
